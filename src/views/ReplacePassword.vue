  ...
  methods: {
    register() {
      if (this.input.fullName !== "" && this.input.email !== "" && this.input.password !== "") {
        ...
        axios.post(`http://localhost:8080/auth/confirmForgotPassword/token`, this.input)
            .then(res => {
...
<template>
  <div class="flex flex-col items-center justify-center min-h-screen bg-white px-4">
    ...
    <div class="content-center mb-14">
      <p class="text-5xl roboto-bold tracking-tight text-balance text-blu800 sm:text-5xl">Reset your account
        password</p>
    </div>
    <form name="register-form"
          class="w-full max-w-md p-6 bg-white rounded-lg border border-gray-300 shadow-md flex flex-col space-y-6 mb-12">
      <!-- Full Name Input -->
      <div class="flex flex-col space-y-2 text-left">
        ...
        <input id="fullName" type="text" v-model="input.fullName"
               class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 outline-none text-sm"
